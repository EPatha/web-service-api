...
class _EditState extends State<Edit> {
  ...
  Future _getData() async {
    try {
      final response = await http.get(Uri.parse(
        // Connect to Docker PHP backend
        // Get detail data with id
        "http://localhost:8080/detail.php?id='${widget.id}'"
      ));
  ...
  Future _onUpdate(context) async {
    try {
      return await http.post(
        Uri.parse("http://localhost:8080/update.php"),
        body: {
          "id": widget.id,
          "title": title.text,
  ...
  Future _onDelete(context) async {
    try {
      return await http.post(
        Uri.parse("http://localhost:8080/delete.php"),
        body: {
          "id": widget.id,
        },
      ).then((value) {
